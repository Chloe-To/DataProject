...
# Load tokenizer once (assuming it's the same for all)
tokenizer = BertTokenizer.from_pretrained("bert-base-uncased") # or any of the subfolders

# Define label types
LABEL_TYPES = ["urgency", "importance", "tone", "sentiment"]

def load_label_encoders():
    return {
        label: joblib.load(f"models/{label}/label_encoder.pkl")
        for label in LABEL_TYPES
    }

def load_models():
    return {
        label: AutoModelForSequenceClassification.from_pretrained(f"models/{label}")
        for label in LABEL_TYPES
    }

def predict_labels(message: str):
    ...
    for label in LABEL_TYPES:
        ...
        with torch.no_grad():
            ...
            decoded_label = label_encoders[label].inverse_transform([pred_class_id])[0]
            predictions[label] = decoded_label
